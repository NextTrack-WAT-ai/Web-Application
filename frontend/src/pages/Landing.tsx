...
export default function Landing() {
  ...
  const getPlaylist = async (playListId: string) => {
    ...
    const response = await fetch(
      `http://localhost:8080/api/playlist?id=${playListId}`,
      {
        method: "GET",
      }
    );
  ...
  const getShuffledPlaylist = async (playlistId: string) => {
    // very long request, use as sparingly as possible
    setLoading(true);
    const response = await fetch(
      `http://localhost:8080/api/playlist/reshuffle?id=${playlistId}`,
      {
        method: "GET",
      }
    );
  ...
  useEffect(() => {
    // updates the playlist to render whenever the selected playlistId changes
    getPlaylist(playlistId);
  }, [playlistId]);

  useEffect(() => {
    // useEffect wrapper for the reshuffle api route
    if (playlistId.length > 0) {
      getShuffledPlaylist(playlistId);
    }
  }, [shuffleTime]); // we call this again each time we click the "reshuffle" button

  return <div>{loading ? <h2>Loading...</h2> : <Stepper />}</div>;
}
